...
class Widget(QWidget):
    ...
    def initUI(self) -> None:
        ...
        grid.addWidget(QLabel('Disc No.', self), 6, 0, 1, 3)
        self.disc = QLineEdit(self)
        grid.addWidget(self.disc, 7, 0, 1, 3)

        grid.addWidget(QLabel('Track No.', self), 8, 0, 1, 3)
        ...
        self.image = None
        self.pixmap = QPixmap('alt.png').scaledToHeight(350)
        ...
        modifyBtn = QPushButton('Modify', self)
    ...
    def showInfo(self) -> None:
        """fill the tags of current page into widgets"""
    ...
    def modifyTags(self) -> None:
        """apply changes to the ID3 tags and filename of the audio file"""
        if not self.select.text():
            self.showMsg(False, 'Select an audio file first!')
            return
        if not self.image:
    ...
    def showMsg(self, info: bool, msg: str) -> None:
        """show information or warning in message box"""
...
def main() -> None:
    ...
    widget.setStyleSheet(
        'QLabel { font-weight: bold; } QLineEdit { margin-right: 5px; }')
    sys.exit(app.exec_())
